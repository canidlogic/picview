...
/*
 * PicViewConfig object
 * ====================
 * 
 * This global object contains configuration settings used by the
 * various PicView classes.  To configure the appearance of the control
 * panel icons, see the separate PicViewIcon object.
 * 
 *     fastPix
 * 
 * The approximate maximum number of pixels that preview images
 * generated by PicViewImage should have.
 * 
 * Should be a finite value that is at least 4.0.  This does not
 * actually have to be an integer value, since it is only an approximate
 * target.
 * 
 * Changes to this value will take effect the next time that an image is
 * loaded.
 * 
 *     scrollLine
 * 
 * When mouse wheel scroll events are generated and their distance is
 * reported in lines rather than pixels, scrollLine stores the number
 * of ems per scroll line, which will then be converted to pixels per
 * scroll line once the em distance is known.
 * 
 * This value is also used if the scroll units reported in the event are
 * some unknown value that is neither pixel nor line nor page.
 * 
 * Should be a finite value that is greater than zero.  Changes to this
 * value will take effect immediately.
 * 
 *     scrollPage
 * 
 * When mouse wheel scroll events are generated and their distance is
 * reported in pages rather than pixels, scrollPage stores the number
 * of ems per scroll page, which will then be converted to pixels per
 * scroll page once the em distance is known.
 * 
 * Should be a finite value that is greater than zero.  Changes to this
 * value will take effect immediately.
 * 
 *     scrollScale
 * 
 * When the mouse wheel is used for zooming in or out of a displayed
 * image, scrollScale controls the sensitivity of the scroll wheel.
 * scrollScale is defined in units of (mag / em), such that when the
 * absolute value of the scroll distance in pixels is converted to ems,
 * the enlargement scaling value will be (1.0 + (ems * scrollScale)).
 * For zoom out, the shrinking scaling value will be one divided by the
 * corresponding enlargement scaling value.
 * 
 * scollScale should be a finite value that is greater than zero.
 * Changes to this value will take effect immediately.
 * 
 *     maxEnlarge
 * 
 * When the user zooms into a displayed picture, there is a limit to how
 * far they can zoom in.  maxEnlarge controls the maximum amount of
 * zoom.  Its minimum value of 1.0 means that pictures can be at most
 * zoomed into 100% scale where the width of one image pixel equals the
 * width of one display pixel.  A value of 2.0 means that pictures can
 * be at most zoomed into 200% scale where the width of one image pixel
 * equals the width of 2.0 display pixels.  And so forth.
 * 
 * Should be a finite value that is at least 1.0.  It does not have to
 * be an integer, although that is a good idea.  Changes to this value
 * will take effect immediately.
 * 
 *     holdFence
 * 
 * For an input hold event to occur, one of the conditions is that the
 * primary pointer may not travel more than a certain distance while it
 * is down, such that the primary pointer stays in approximately the
 * same place.  holdFence stores the maximum number of ems that the
 * primary pointer may travel for it to still be eligible for generating
 * a hold event.
 * 
 * Should be a finite value that is greater than or equal to zero.
 * Changes to this value will take effect immediately.
 * 
 *     holdTime
 * 
 * An input hold event may only occur after the primary pointer has been
 * held down for a certain length of time.  holdTime is the number of
 * milliseconds that the pointer must be down before a hold event is
 * possibly triggered.
 * 
 * Should be a finite value that is greater than zero.  This value will
 * be passed through as a delay value to setTimeout().  Changes to this
 * value take effect the next time that the primary pointer is pressed
 * down.
 * 
 *     actionFence
 * 
 * In any state except when the displayed image is zoomed in, the user
 * may swipe left, right, up, or down to trigger actions.  actionFence
 * is the distance in ems the primary pointer must travel before the
 * drag will be interpreted as a swipe action.
 * 
 * Should be a finite value that is greater than zero.  Changes to this
 * value will take effect immediately.
 */

window.PicViewConfig = {
  "fastPix"     : 76800,
  "scrollLine"  : 1.5,
  "scrollPage"  : (1.5 * 5.0),
  "scrollScale" : (0.5 / 6.0),
...
PicViewImage.prototype._handleImageLoad = function() {
  ...
  if (this._img.complete &&
    ...
    if (this._img.naturalWidth * this._img.naturalHeight
      ...
      h = Math.floor(Math.sqrt(PicViewConfig.fastPix / ar));
      w = Math.floor(h * ar);
...
PicViewInputDriver.prototype._handleTouchDown = function(tid, cx, cy) {
  ...
  if (primary) {
    ...
    this._htime = setTimeout(function() {
      ...
      // Compute the SQUARE of the hold threshold distance
      dthr = PicViewConfig.holdFence * selfref._em;
      dthr = dthr * dthr;
      
      // Ignore the timeout if we ever went into double mode or if we
      // ever strayed further than the threshold
      if (selfref._dbl || (selfref._bmaxd > dthr)) {
        return;
      }
...
PicViewInputDriver.prototype._handleScroll = function(ev) {
  ...
  if (ev.deltaMode === WheelEvent.DOM_DELTA_LINE) {
    dv = dv * PicViewConfig.scrollLine;
  
  } else if (ev.deltaMode === WheelEvent.DOM_DELTA_PAGE) {
    dv = dv * PicViewConfig.scrollPage;
...
PicView.prototype._inputDrag = function(x, y) {
  ...
  if (this._dragstate.action !== undefined) {
    ...
    // If square of distance has exceeded the square of the action
    // fence, then handle an action
    f = PicViewConfig.actionFence * this._em;
    if (d >= f * f) {
      // Set the action flag
...
PicView.prototype._inputZoom = function(d) {
  ...
  if (this._view.zoom(
          d, this._dragstate.curd,
          PicViewConfig.maxEnlarge, this._fullv)) {
    if (!this._requested) {
      // Set a call to a fast-preview repaint that will run sometime
      // later during event handling and set requested flag
      this._requested = true;
...
PicView.prototype._inputScroll = function(s) {
  ...
  // Ignore if scroll distance is zero
  if (s === 0.0) {
    return;
  }
  
  // Compute zoom-in scaling value using the absolute value
  sv = 1.0 + ((Math.abs(s) / this._em) * PicViewConfig.scrollScale);
  
  // If scroll distance was greater than zero, it's a scroll down, so we
  // need to zoom OUT; invert the scaling value in this case
  if (s > 0.0) {
    sv = 1.0 / sv;
  }
  ...
  if (this._view.zoom(
          sv, 1.0, PicViewConfig.maxEnlarge, this._fullv)) {
    if (!this._requested) {
